...
def _new_block(i: int, btype: BlockType, text: str, parent_id: str | None) -> DocBlock:
    return DocBlock(
        block_id=f"#/vlm/{i}",
        type=btype,
        text=text,
...
def markdown_to_blocks(md: str) -> list[DocBlock]:
    lines = md.splitlines()
    ...
    def flush_para() -> None:
        ...
        if para_buf:
            ...
            if text:
                blocks.append(_new_block(i, BlockType.PARAGRAPH, text, parent_for(99)))
                i += 1
            para_buf.clear()
    ...
    for raw in lines:
        ...
        if m:
            ...
            block = _new_block(i, btype, text, parent_for(level))
            blocks.append(block)
        ...
        if lm:
            flush_para()
            blocks.append(_new_block(i, BlockType.LIST_ITEM, lm.group(1).strip(), parent_for(99)))
            i += 1
            continue
